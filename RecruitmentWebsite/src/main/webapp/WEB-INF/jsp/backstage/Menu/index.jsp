...
<link rel="stylesheet" type="text/css"
	href="backstagestyle/Css/bootstrap.css" />
<link rel="stylesheet" type="text/css"
	href="backstagestyle/Css/bootstrap-responsive.css" />
<link rel="stylesheet" type="text/css"
	href="backstagestyle/Css/style.css" />
...
<body>
	<form class="form-inline definewidth m20"
		action="toMenuIndex.controller" method="post" id="searchForm">
		职位名称： <input type="text" name="oname" id="menuname"
			class="abc input-default" placeholder="输入查询条件" value="">&nbsp;&nbsp;
		...
		<c:forEach items="${zhiwei}" var="item">
			<c:forEach items="${item.occupation}" var="item2">
				<tr class="del${item2.oid }">
					...
					<td>${item.cname}</td>
					<td>${item2.oname}</td>
					<td><fmt:formatNumber value="${item2.omaxsalary}"
							pattern="#.0k"></fmt:formatNumber>--<fmt:formatNumber
					...
					<td><fmt:formatDate value="${item2.orelease}"
							pattern="yyyy-MM-dd" /></td>
					<td><a href="toDetailOcc.controller?oid=${item2.oid}">查看详情</a>&nbsp;&nbsp;<a
						href="toxiaxian.controller?oid=${item2.oid}"> <c:if
								test="${item2.ostate eq 1}">已下线</c:if> <c:if
								test="${item2.ostate eq 0}">强制下线</c:if>
					</a>&nbsp;&nbsp;<a href="javascript:void(0)"
						onclick="deleteOcc(${item2.oid})">删除</a></td>
				</tr>
			</c:forEach>
		</c:forEach>
