...
public interface ChatrecordMapper {
	...
	@Insert("insert into Chatrecord value(null,#{chatrecord.ruserid.userid},#{chatrecord.iuserid},NOW(),#{chatrecord.crtext},#{chatrecord.crqf})")
	int insertChatrecord(@Param("chatrecord")Chatrecord chatrecord);

	int updateChatrecord(@Param("chatrecord")Chatrecord chatrecord);
	
	@Delete("delete from chatrecord where ruserid=#{chatrecord.ruserid.userid} and iuserid=#{chatrecord.iuserid}")
